...
impl<T: Config> Pallet<T> {
	pub fn check_task(now: BlockNumberFor<T>, remaining_weight: Weight) -> Weight {
		const MAX_TASKS_TO_PROCESS: usize = 5;
		// reduce the weight used to read the task list
		remaining_weight.saturating_sub(<T as Config>::WeightInfo::remove_task());
		let cancel_weight = <T as Config>::WeightInfo::cancel();
