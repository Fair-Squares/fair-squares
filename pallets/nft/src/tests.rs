...
fn burn_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		// not allowed in Permissions
		assert_noop!(
			NFTPallet::burn(RuntimeOrigin::signed(BOB), PossibleCollections::OFFICESTEST, ITEM_ID_0),
			Error::<Test>::NotPermitted
		);
		...
		// not existing
		assert_noop!(
			NFTPallet::burn(RuntimeOrigin::signed(CHARLIE), PossibleCollections::HOUSESTEST, ITEM_ID_0),
			Error::<Test>::ItemUnknown
		);
	});
}
...
fn destroy_collection_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(NFTPallet::mint(RuntimeOrigin::signed(BOB), PossibleCollections::HOUSESTEST, metadata));

		// existing item
		assert_noop!(
			NFTPallet::destroy_collection(RuntimeOrigin::signed(CHARLIE), PossibleCollections::HOUSESTEST),
		...
		// not allowed in Permissions
		assert_noop!(
			NFTPallet::destroy_collection(RuntimeOrigin::signed(BOB), PossibleCollections::OFFICESTEST),
			Error::<Test>::NotPermitted
		);
